  ...
  <div class="flex flex-col min-h-screen bg-white py-4">
    ...
    <div v-if="tampilanAktif === 'daftar'" class="px-6 mt-12">
      ...
      <div class="flex flex-col w-full mt-6 space-y-4">
        <h1 class="text-2xl font-bold text-[#EB5523]">Highlight Portfolios</h1>
        ...
        <form @submit.prevent="updatePortfolio" class="space-y-4">
          ...
          <div>
            ...
            <div v-if="imagePreviewUrl" class="mb-4">
              ...
              <img :src="imagePreviewUrl" alt="Pratinjau Gambar" class="w-full max-w-xs h-auto rounded-lg object-cover border" />
            </div>
          ...
          <div>
            ...
            <Editor
              v-model="portfolio.description"
              editorStyle="height: 320px"
              :pt="{ toolbar: { style: { backgroundColor: 'white', borderColor: '#D1D5DB' } } }"
              class="mt-1 block w-full shadow-sm border border-gray-700 rounded-2xl bg-white overflow-hidden"
            />
          </div>
...
export default {
  // Komponen juga di-auto-import oleh Nuxt, jadi blok ini bisa dikosongkan jika tidak ada konfigurasi khusus.
  components: {},
  ...
  data() {
    return {
      ...
      // [NEW] 'value' untuk v-model search bar, 'searchTimeout' untuk debouncing
      value: "",
      searchTimeout: null,
    };
  },
  ...
  methods: {
    ...
    async fetchPortfolios(searchTerm = "") {
      console.log(`Mencoba mengambil data portfolios. Term: "${searchTerm}"`);
      try {
        // API URL dasar
    ...
    async submitPortfolio() {
      ...
      try {
        const response = await this.$api.post("/portfolios/", formData, {
          headers: {
    ...
    async handleEdit(slug) {
      try {
        // 1. Panggil API untuk mendapatkan detail portfolios
    ...
    async updatePortfolio() {
      ...
      try {
        ...
        const response = await this.$api.put(`/portfolios/${this.portfolio.slug}`, formData, {
    ...
    async fetchHighlighted() {
      try {
        ...
        const response = await this.$api.get(apiUrl);
        console.log("Data highlited portfolios berhasil diambil:", response.data.highlightedPortfolios);
    ...
    async deleteCard(portoSlug) {
      try {
        let apiUrl = `/portfolios/${portoSlug}`;
    ...
    async deletePortHighlight(portoId) {
      try {
        let apiUrl = `/content-tracking/highlighted-portfolios/${portoId}`;
